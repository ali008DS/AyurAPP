...
export default function CurrentMonth() {
  ...
  const updateItem = (
    ...
    // Special handling for responsibility toggle
    if (field === "responsibilty") {
      // Only allow toggle if current user is the responsibleUser
      ...
      if (value === true && !updatedItems[index].responsibilty) {
        // Toggling ON from OFF - assign current user ID (as string)
        updatedItems[index] = {
          ...updatedItems[index],
          responsibilty: currentUserId,
        };
      } else if (value === false) {
        // Toggling OFF - remove the field (set to undefined)
        updatedItems[index] = {
          ...updatedItems[index],
          responsibilty: undefined,
        };
      }
      // If already ON (has string/object), do nothing - preserve existing value
    } else {
      // Normal field update
