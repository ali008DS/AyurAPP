...
export default function PreviousMonths({}: PreviousMonthsProps) {
  ...
  const fetchInventory = async () => {
    try {
      const response = await ApiManager.getMedicinePreparationInventory();
      if (response.data) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await ApiManager.deleteMedicinePreparationInventory(id);
            ...
            >
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                  ...
                  >
                    Monthly Medicine Preparation Inventory
                  </Typography>
                  <Chip
