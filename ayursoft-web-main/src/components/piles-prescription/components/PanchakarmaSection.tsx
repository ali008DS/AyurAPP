...
export const PanchakarmaSection = () => {
  const { prescriptionData, updatePrescriptionData } = usePilesPrescription();
   const [panchakarmaGroups, setPanchakarmaGroups] = React.useState<PanchakarmaGroup[]>([]);
  const [availablePanchakarmas, setAvailablePanchakarmas] = React.useState<string[]>([]);
  ...
  // Sync form with prescriptionData when it changes (e.g., when loading draft)
  useEffect(() => {
    if (prescriptionData.panchakarma && prescriptionData.panchakarma.length > 0) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAvailablePanchakarmas(therapiesRes.data.map((item: { name: string }) => item.name));
    ...
    if (updateCounter > 0) {
      updatePrescriptionData({
        panchakarma: watch('panchakarma').filter(
          (item: Panchakarma) => item.therapy || item.oil || item.quantity || item.duration || item.days
        ),
      });
    }
          ...
          <Box sx={{ borderRadius: 1 }}>
            ...
            <Box sx={{ mt: 2, display: "flex", alignItems: "center" }}>
              <IconButton
                ...
                onClick={() => {setDialogOpen(true);console.log(fields)}}
                sx={{
                  mr: 1,
                  backgroundColor: "white",
                  color: "#444",
