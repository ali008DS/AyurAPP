...
import { MedicineWithStock } from "../../department-management/types";
...
export const MedicinesPrescribedSection = () => {
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      try {
        const response: { data: any[] } = await ApiManager.getStockMedicine();
        ...
        console.log("mapped medicines:", stockList);
      } catch (error) {
  ...
  const handleMedicineChange =
    (index: number, field: keyof Medicine) =>
      (_: React.SyntheticEvent, newValue: MedicineWithStock | string | null) => {
        const updatedMedicines = [...prescriptionData.medicines];
        updatedMedicines[index] = {
          ...updatedMedicines[index],
          [field]: typeof newValue === "object" ? newValue?.name || "" : newValue,
        };
