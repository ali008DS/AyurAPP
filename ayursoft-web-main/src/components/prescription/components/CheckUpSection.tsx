...
export const CheckUpSection = () => {
  ...
  const handleTestNameChange = (
    ...
    // Set prices from testsList if available
    const found = testsList.find((t) => t.name === value);
    newTest[centerIndex].test[testIndex].marketPrice = found ? found.marketPrice : undefined;
    newTest[centerIndex].test[testIndex].discountedPrice = found ? found.discountedPrice : undefined;
    // For manually typed tests, make sure we initialize description and notes
    // to ensure they are included in the API payload
