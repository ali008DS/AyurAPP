...
function ProgressNotes({ open, onClose, data }: ProgressNotesProps) {
  ...
  const onSubmit = async (formData: FormValues) => {
    ...
    try {
      ...
      if (existingData) {
        ...
        await ApiManager.updateProgressNote(existingData._id, payload);
        setAlert({
          severity: "success",
        ...
        // Create new record
        await ApiManager.createProgressNote(payload);
        setAlert({
          severity: "success",
      ...
      setAlert({
        severity: "error",
        message: error?.message || "Failed to save Progress Notes",
      });
    } finally {
      ...
      <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
        <DialogContent sx={{ pt: 0, pb: 0.5 }}>
          {loading ? (
            ...
            <>
              <Box sx={{ maxHeight: "98vh", overflowY: "auto" }}>
