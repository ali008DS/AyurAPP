...
function MedOpenDrawer({ open, onClose, recall }: MedOpenDrawerProps) {
  ...
  // Use panchakarma-specific opening-stock endpoint
  await ApiManager.createPanchakarmaOpeningStock(openingStockData);
  ...
  return (
    <BootstrapDialog fullWidth maxWidth="md" open={open} onClose={onClose}>
      <DialogTitle sx={{ m: 0, p: 2 }}>Open Stock</DialogTitle>
      <IconButton
        aria-label="close"
        onClick={onClose}
        sx={(theme) => ({
