...
interface MedPurchaseDrawerProps {
  ...
  manufacturer?: any | null; // Optional prop for editing
}
...
function AddMedicineDialog({ open, onClose, recall, manufacturer }: MedPurchaseDrawerProps) {
  ...
  const onSubmit = async (data: MedicineFormType) => {
    try {
      await ApiManager.createMedicine(data);
      setAlert({
        severity: "success",
  ...
  return (
    <BootstrapDialog fullWidth maxWidth="sm" open={open} onClose={onClose}>
      <DialogTitle sx={{ m: 0, p: 2 }}>Add Medicine</DialogTitle>
      <IconButton
        aria-label="close"
        onClick={onClose}
        sx={(theme) => ({
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <Autocomplete
                    ...
                    value={manufacturers.find((m) => m._id === value) || null}
                    onChange={(_, newValue) => {
