...
function TodaysGeneralOPD() {
  ...
  const fetchPrescriptions = async () => {
    try {
      const response = await ApiManager.getTodaysGeneralPrescriptions();
      ...
      console.error("Failed to fetch today's general prescriptions", error);
    } finally {
      setLoading(false);
    }
  };
          ...
          <Box display="flex" gap={1} justifyContent="center" p={0.2}>
            {params.row.status === "pending" && (
              <Button
                onClick={() =>
                  navigate(
                    `/general-prescription/create/${patientId}/${prescriptionId}`
                  )
                }
                variant="outlined"
                size="small"
            ...
            {params.row.status === "completed" && (
              <Button
                onClick={() =>
                  navigate(`/general-prescription/edit/${patientId}/${prescriptionId}`)
                }
                variant="outlined"
                color="warning"
                size="small"
  ...
  const handleDelete = async (id: string) => {
    try {
      await ApiManager.deleteGeneralPrescription(id);
  ...
  return (
    <Box sx={{ px: 3 }}>
      <HeadingText name="Today's General Prescriptions" />
...
export default TodaysGeneralOPD;
