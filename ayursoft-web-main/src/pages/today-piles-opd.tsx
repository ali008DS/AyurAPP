...
function TodayPilesOPD() {
  ...
  const fetchPrescriptions = async () => {
    try {
      const response = isAdmin
        ? await ApiManager.getTodaysPilesPrescriptions()
        : await ApiManager.getAllPilesPrescriptions(user?.id ? user.id : "");
          ...
          <Box display="flex" gap={1} justifyContent="center" p={0.2}>
            {params.row.status === "pending" && (
              <Button
                onClick={() =>
                  navigate(
                    `/piles-prescription/create/${patientId}/${prescriptionId}`
                  )
                }
                variant="outlined"
                size="small"
            ...
            {params.row.status === "completed" && (
              <Button
                onClick={() =>
                  navigate(
                    `/piles-prescription/edit/${patientId}/${prescriptionId}`
                  )
                }
                variant="outlined"
                color="warning"
  ...
  const handleDelete = async (id: string) => {
    try {
      await ApiManager.deletePilesPrescription(id);
