...
import RegRoomAssignment from "../../../components/registers/regRoomAssignment";
import MedicineRoom from "../register-room-medicine-preparation/MedicineRoom"
...
function UtensilsRegRoom() {
  ...
  const handleSubmit = async () => {
    try {
      const payload = {
        name: formData.name,
        utensilType: "medicinePreparation",
      };
  ...
  return (
    <Box sx={{ px: 3 }}>
      <HeadingText name="Medicine Preparation Management" />
      ...
      {tabValue === 1 && <RegRoomAssignment />}

      {tabValue === 2 && <MedicineRoom />}
    </Box>
  );
}

export default UtensilsRegRoom;
